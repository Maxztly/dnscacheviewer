...
def execute_ipconfig():
    ...
    try:
        with open("ipconfig_output.txt", "wb") as output_file:
            result = subprocess.run(["ipconfig", "/displaydns"], stdout=output_file, stderr=subprocess.PIPE)
        with open("ipconfig_output.txt", "rb") as output_file:
            output_data = output_file.read()
            output_text.insert(tk.END, output_data.decode(errors='ignore'))
    except Exception as e:
        output_text.insert(tk.END, f"Fehler beim Ausführen von 'ipconfig /displaydns': {str(e)}")

root = tk.Tk()
root.title("DNS-CACHE-VIEWER")
...
execute_button = tk.Button(root, text="EXPLORE: DNS-CACHE", command=execute_ipconfig, bg="black", fg="lime green")
